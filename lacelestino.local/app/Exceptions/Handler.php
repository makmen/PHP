...
class Handler extends ExceptionHandler
{
    ...
    private function preperViewError() {
        $obj = new \App\Http\Controllers\AppController(
            ...
            new \App\Repositories\ArticleRepository(new \App\Article),
            new \App\Repositories\PortfolioRepository(new \App\Portfolio)
        );
        $navigation = view('index.navigation')->with('menu', $obj->buildMenu() )->render();
        $footerArticles = $obj->getArticleRepository()->getSeveralArticles( Config::get( 'settings.footer_articles' ) );
        $footerPortfolios = $obj->getPortfolioRepository()->getSeveralPortfolios( Config::get( 'settings.footer_portfolios' ));
        $footer = view('index.footer')->
            with([
                'footerArticles'=> $footerArticles,
                'footerPortfolios'=> $footerPortfolios,
            ])->render();
        $out = [ 'navigation' => $navigation, 'footer' => $footer ];
