...
<?  } else {  ?>
<form action="http://framework.local/vequipment/view" method="post"  class="cmxform">
<div class="account">
...
<?  } else {  ?>
    <? if ( !empty(compiler::getInstance()->vars["vequipment"])) { ?>
<div class="ambitios_p4">
	<div class="ambitios_wrapper ambitios_p2">
            <h3 class="ambitios_uppercase ambitios_p5"><? echo compiler::getInstance()->vars["vequipment"]['title']; ?></h3>
            <div class="ambitios_date">
                <? echo compiler::getInstance()->vars["vequipment"]['created']; ?>
                <? if ( compiler::getInstance()->vars["_SESSION"]['group'] == 1) { ?>
                <div class="newsedit">
                        <a href="http://framework.local/vequipment/edit/19"  onClick="back();">Редактировать</a>
                </div>
                <div class="cb"></div>
                <?  }  ?>
            </div>
                <br />
                <p class="price">Цена: <span><? echo compiler::getInstance()->vars["vequipment"]['price']; ?> BYR</span></p>
                <? echo compiler::getInstance()->vars["vequipment"]['content']; ?>
                <? if ( compiler::getInstance()->vars["vequipment"]['photo'] != '') { ?>
                    <div class="ambitios_indent">
                            <img src="<? echo compiler::getInstance()->vars["vequipment"]['photo']; ?>" width="450" height="340" alt="" />
                    </div>
                    <br />
                <?  }  ?>
                <? if ( (!empty(compiler::getInstance()->vars["powers"]))) { ?>
                    <p class="technical">Источники питания:</p>
                    <ul>
                    <? if (is_array(compiler::getInstance()->vars["powers"]) && count (compiler::getInstance()->vars["powers"])>0) {reset(compiler::getInstance()->vars["powers"]); while (list(compiler::getInstance()->vars["k"], compiler::getInstance()->vars["v"]) = each(compiler::getInstance()->vars["powers"])) {  ?>
                        <li><a href="http://framework.local/power/view/<? echo compiler::getInstance()->vars["v"]['id']; ?>"><? echo compiler::getInstance()->vars["v"]['title']; ?></a></li>
                    <?  } }  ?>
                    </ul>
                    <br />
                <?  }  ?>
                <p class="technical">Основные технические данные:</p>
                <ul><? echo compiler::getInstance()->vars["vequipment"]['technical_data']; ?></ul>
                <br />
                <p class="technical">Состав установки:</p>
                <ul><? echo compiler::getInstance()->vars["vequipment"]['composition']; ?></ul>
	</div>
	<a href="http://framework.local/vequipment">Список оборудования</a>
</div>
