...
<?  } else {  ?>
<form action="http://framework.local/account/register" method="post"  class="cmxform">
<div class="account">
...
<?  } else {  ?>
    <h3 class="register"><? if ( compiler::getInstance()->vars["mode"] =='add') { ?>Регистрация <? if ( !isset(compiler::getInstance()->vars["_SESSION"]['group'])) { ?>пользователя<?  } else {  ?>администратора<?  }  ?><?  } else {  ?>Изменение данных пользователя<?  }  ?></h3>
<? if ( compiler::getInstance()->vars["errs"]['message']) { ?> 
	<div class="mess-top">
		<div class="error"><div class="msg"><? echo compiler::getInstance()->vars["errs"]['message']; ?></div></div>
	</div>
<?  }  ?>
<? if ( isset(compiler::getInstance()->vars["reg_ok"])) { ?>
	<div class="mess-top">
		<div class="fbok">
			<div class="success"><? if ( compiler::getInstance()->vars["mode"] =='add') { ?>Регистрация завершена успешно<?  } else {  ?>Данные сохранены<?  }  ?></div>
		</div>
	</div>
<?  } else {  ?>
<div>
	<form id="register" method="post" action="">
		<div class="fb">
			...
			<div class="date">
				<input class="w100<? if ( isset(compiler::getInstance()->vars["errs"]['name'])) { ?> err<?  }  ?>" type="text" name="name" value="<? echo compiler::getInstance()->vars["name"]; ?>" />
				<? if ( isset(compiler::getInstance()->vars["errs"]['name'])) { ?>
					<div class="error">
						<div class="msg"><? echo compiler::getInstance()->vars["errs"]['name']; ?></div>
					</div>
				<?  }  ?>
			</div>
			<div class="cb"></div>
		...
		<div class="fb">
			...
			<div class="date">
				<input class="w100<? if ( isset(compiler::getInstance()->vars["errs"]['middlename'])) { ?> err<?  }  ?>" type="text" name="middlename" value="<? echo compiler::getInstance()->vars["middlename"]; ?>" />
				<? if ( isset(compiler::getInstance()->vars["errs"]['middlename'])) { ?>
					<div class="error">
						<div class="msg"><? echo compiler::getInstance()->vars["errs"]['middlename']; ?></div>
					</div>
				<?  }  ?>
			</div>
			<div class="cb"></div>
		...
		<div class="fb">
			...
			<div class="date">
				<input class="w100<? if ( isset(compiler::getInstance()->vars["errs"]['lastname'])) { ?> err<?  }  ?>" type="text" name="lastname" value="<? echo compiler::getInstance()->vars["lastname"]; ?>" />
				<? if ( isset(compiler::getInstance()->vars["errs"]['lastname'])) { ?>
					<div class="error">
						<div class="msg"><? echo compiler::getInstance()->vars["errs"]['lastname']; ?></div>
					</div>
				<?  }  ?>
			</div>
			<div class="cb"></div>
		...
		<div class="fb">
			...
			<div class="date">
                            <input class="w100<? if ( isset(compiler::getInstance()->vars["errs"]['email'])) { ?> err<?  }  ?>" type="text" name="email" value="<? echo compiler::getInstance()->vars["email"]; ?>" />
                            <? if ( isset(compiler::getInstance()->vars["errs"]['email'])) { ?>
                                <div class="error">
                                    <div class="msg"><? echo compiler::getInstance()->vars["errs"]['email']; ?></div>
                                </div>
                            <?  }  ?>
			</div>
			<div class="cb"></div>
		...
		<div class="fb">
			...
			<div class="date">
				<input class="w100<? if ( isset(compiler::getInstance()->vars["errs"]['phone'])) { ?> err<?  }  ?>" type="text" name="phone" value="<? echo compiler::getInstance()->vars["phone"]; ?>" />
			<? if ( isset(compiler::getInstance()->vars["errs"]['phone'])) { ?>
				<div class="error">
					<div class="msg"><? echo compiler::getInstance()->vars["errs"]['phone']; ?></div>
				</div>
			<?  }  ?>
			</div>
			<div class="cb"></div>
		</div>
		<? if ( compiler::getInstance()->vars["mode"] =='add') { ?>
		<div class="fb">
			<div class="description">Логин:</div>
			<div class="date">
				<input class="w100<? if ( isset(compiler::getInstance()->vars["errs"]['login'])) { ?> err<?  }  ?>" type="text" name="login" value="<? echo compiler::getInstance()->vars["login"]; ?>" />
				<? if ( isset(compiler::getInstance()->vars["errs"]['login'])) { ?>
					<div class="error">
						<div class="msg"><? echo compiler::getInstance()->vars["errs"]['login']; ?></div>
					</div>
				<?  }  ?>
			</div>
			<div class="cb"></div>
		...
		<div class="fb">
			...
			<div class="date"><input class="w100<? if ( isset(compiler::getInstance()->vars["errs"]['pass'])) { ?> err<?  }  ?>" type="password" name="pass" value="<? echo compiler::getInstance()->vars["pass"]; ?>" />
			<? if ( isset(compiler::getInstance()->vars["errs"]['pass'])) { ?>
				<div class="error">
					<div class="msg"><? echo compiler::getInstance()->vars["errs"]['pass']; ?></div>
				</div>
			<?  }  ?>
			</div>
			<div class="cb"></div>
		...
		<div class="fb">
			...
			<div class="date"><input class="w100<? if ( isset(compiler::getInstance()->vars["errs"]['repass'])) { ?> err<?  }  ?>" type="password" name="repass" value="<? echo compiler::getInstance()->vars["repass"]; ?>" />
			<? if ( isset(compiler::getInstance()->vars["errs"]['repass'])) { ?>
				<div class="error">
					<div class="msg"><? echo compiler::getInstance()->vars["errs"]['repass']; ?></div>
				</div>
			<?  }  ?>
			</div>
			<div class="cb"></div>
		</div>
                    <? if ( !isset(compiler::getInstance()->vars["_SESSION"]['group'])) { ?>
                        <div class="fbcaptaha">
                            <div class="description">Код подтверждения:</div>
                            <div class= "image">
                                    <img id="captchaDiv" src="http://framework.local/account/makecaptcha" alt="" />
                                    <img id="refresh" src="http://framework.local/images/refresh.gif" alt="" />
                            </div>
                            <div class="date"><input class="w100<? if ( isset(compiler::getInstance()->vars["errs"]['key'])) { ?> err<?  }  ?>" type="text" name="key" value="<? echo compiler::getInstance()->vars["key"]; ?>" />
                            <? if ( isset(compiler::getInstance()->vars["errs"]['key'])) { ?>
                                    <div class="error">
                                            <div class="msg"><? echo compiler::getInstance()->vars["errs"]['key']; ?></div>
                                    </div>
                            <?  }  ?>
                            </div>
                            <div class="cb"></div>
                    ...
                    <?  }  ?> 
		<?  }  ?> 
		<div class="submit <? if ( compiler::getInstance()->vars["mode"] =='edit') { ?>news<?  }  ?> ">
    ...
    <script type="text/javascript">
        $('#refresh').click(function(){ 
                $('#captchaDiv').attr('src', 'http://framework.local/account/makecaptcha?r='+Math.random());
        });
    </script> 
    <?  }  ?>
    <script type="text/javascript">
        $(function(){  
            $('input[name=email]').change(function() { 
                var url = "http://framework.local/account/checkmail";
