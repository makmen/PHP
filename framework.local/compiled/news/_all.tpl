...
<?  } else {  ?>
<form action="http://framework.local/news/all" method="post"  class="cmxform">
<div class="account">
...
<?  } else {  ?>
    <? if ( !empty(compiler::getInstance()->vars["news"])) { ?>
    <? if (is_array(compiler::getInstance()->vars["news"]) && count (compiler::getInstance()->vars["news"])>0) {reset(compiler::getInstance()->vars["news"]); while (list(compiler::getInstance()->vars["k"], compiler::getInstance()->vars["v"]) = each(compiler::getInstance()->vars["news"])) {  ?>
    <div class="ambitios_p4">
        ...
        <div class="ambitios_wrapper">
            <a href="http://framework.local/news/view/<? echo compiler::getInstance()->vars["v"]['id']; ?>" class="ambitios_button_small_rev ambitios_fleft">Читать</a>
        </div>
    </div>
    <?  } }  ?>
    ...
    <? if (is_array(compiler::getInstance()->vars["pagging"]) && count (compiler::getInstance()->vars["pagging"])>0) {reset(compiler::getInstance()->vars["pagging"]); while (list(compiler::getInstance()->vars["k"], compiler::getInstance()->vars["v"]) = each(compiler::getInstance()->vars["pagging"])) {  ?>
        <a href="http://framework.local/news/all?p=<? echo compiler::getInstance()->vars["v"]; ?>" <? if ( compiler::getInstance()->vars["page"] == compiler::getInstance()->vars["v"]) { ?>class="currentpagging"<?  }  ?>><? echo compiler::getInstance()->vars["v"]; ?></a>&nbsp;
    <?  } }  ?>
</div> 
        <?  }  ?>
    <?  } else {  ?>
